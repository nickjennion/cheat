...
- The Rich splash can now co-brand with Victoria University. A **`SPLASH_DESIGN`**
  ...
  - **`lockup`** — compact VU diamond + `VICTORIA UNIVERSITY` badge beside the bars.
  - **`stacked`** — full VU lockup (large diamond over the `VICTORIA UNIVERSITY`
    wordmark) beside the bars, bars top-aligned with the diamond.
  - **`generic`** — original Cisco-only splash, no VU branding.
- Co-brand designs append a **`× Victoria University`** tag to the
  `CISCO · DNA CENTER` wordmark, riding the same cyan→white gradient. VU
