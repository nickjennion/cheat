...
def test_action_exists_with_expected_signature():
    import main_latest
    assert callable(main_latest.action_ip_mac_export)
    sig = inspect.signature(main_latest.action_ip_mac_export)
...
def test_menu_5_offers_the_new_export():
    import main_latest
    src = inspect.getsource(main_latest.menu_5)
...
def test_prompt_vlans_accepts_space_separated(monkeypatch):
    ...
    assert main_latest._prompt_vlans() == ["900", "905"]
...
def test_prompt_vlans_rejects_non_numeric(monkeypatch):
    ...
    assert main_latest._prompt_vlans() == []
...
def test_timestamped_excel_path_lands_in_the_report_dir(tmp_path, monkeypatch):
    ...
    out = Path(main_latest._timestamped_excel_path("av-export.xlsx"))
...
def test_timestamped_excel_path_strips_any_given_extension(tmp_path, monkeypatch):
    import main_latest
    monkeypatch.chdir(tmp_path)
    out = Path(main_latest._timestamped_excel_path("report.xlsx"))
